...
from schemas.notes_schema import NoteUpdateResponse,Note,NoteCreate,NoteUpdate
...
def create_note(note: NoteCreate, user_id: int = Query(...), api_key: str = Query(...), db: Session = Depends(get_db)):
...
def update_note(id: int, note_update: NoteUpdate, user_id: int = Query(...), api_key: str = Query(...), db: Session = Depends(get_db)):
...
@router.delete("/notes/{id}", response_model=NoteUpdateResponse)
